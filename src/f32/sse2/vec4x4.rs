...
#[inline(always)]
#[must_use]
pub const fn vec4x4(x: [f32; 4], y: [f32; 4], z: [f32; 4], w: [f32; 4]) -> Vec4X4 {
    Vec4X4::new(x, y, z, w)
}
...
impl Vec4X4 {
    ...
    pub const X: Self = Self::new([1.0; 4], [0.0; 4], [0.0; 4], [0.0; 4]);

    pub const Y: Self = Self::new([0.0; 4], [1.0; 4], [0.0; 4], [0.0; 4]);

    pub const Z: Self = Self::new([0.0; 4], [0.0; 4], [1.0; 4], [0.0; 4]);

    pub const W: Self = Self::new([0.0; 4], [0.0; 4], [0.0; 4], [1.0; 4]);

    pub const NEG_X: Self = Self::new([-1.0; 4], [0.0; 4], [0.0; 4], [0.0; 4]);

    pub const NEG_Y: Self = Self::new([0.0; 4], [-1.0; 4], [0.0; 4], [0.0; 4]);

    pub const NEG_Z: Self = Self::new([0.0; 4], [0.0; 4], [-1.0; 4], [0.0; 4]);

    pub const NEG_W: Self = Self::new([0.0; 4], [0.0; 4], [0.0; 4], [-1.0; 4]);

    #[inline(always)]
    #[must_use]
    pub const fn new(x: [f32; 4], y: [f32; 4], z: [f32; 4], w: [f32; 4]) -> Self {
        unsafe {
            Self {
                x: UnionCast { a: x }.v,
                y: UnionCast { a: y }.v,
    ...
    #[inline]
    #[must_use]
    pub const fn splat(x: f32, y: f32, z: f32, w: f32) -> Self {
        unsafe {
            Self {
                x: UnionCast { a: [x; 4] }.v,
                y: UnionCast { a: [y; 4] }.v,
    ...
    #[inline]
    #[must_use]
    pub const fn splat_vec(v: Vec4) -> Self {
        let v = v.to_array();
        unsafe {
            Self {
    ...
    pub fn dot(self, rhs: Self) -> [f32; 4] {
        unsafe {
            let dot = dot4x4(self.x, self.y, self.z, self.w, rhs.x, rhs.y, rhs.z, rhs.w);
            UnionCast { v: dot }.a
        }
    }
}
